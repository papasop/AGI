...
The reconstruction requires the `global_parameterization.json` produced by the
formally certified local response-curve calculation in the companion
Projective-Jet repository.  Its canonical JSON hash must equal the frozen hash
shown above.  The input is intentionally not regenerated from finite-error or
\(L_6\) outcomes.
