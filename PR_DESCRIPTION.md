...
- Replaced the long README development log with a single status table, theorem
  summary, finite-continuation summary, claim boundary, and three reproduction
  entry points.
...
- Added wrapper scripts under `scripts/` for the three README reproduction
  commands.
...
- `python3 scripts/reproduce_finite_chain.py`
- `git diff --check`
