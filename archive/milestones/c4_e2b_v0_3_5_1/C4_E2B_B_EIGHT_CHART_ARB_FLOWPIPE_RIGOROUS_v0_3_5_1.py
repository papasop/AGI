#!/usr/bin/env python3
"""C4-E2b-B: rigorous outward-rounded Arb parent for C4-E2b v0.3.5.1.
...
COMMIT = "805ef2e62cdaec81f744f881d16806e74121b696"
...
TITLE = "C4-E2b-B RIGOROUS OUTWARD-ROUNDED ARB PARENT"
VERSION = "0.3.5.1"
ENCLOSURE_RIGOUR_GUARDS = {
    "quadratic_radius_arithmetic": "unpatched",
    "neumann_norm_arithmetic": "unpatched",
    "neumann_tail_arithmetic": "unpatched",
}
...
def arb_upper_point(x):
    """Return an Arb point at an already outward Arb upper endpoint."""
    return arb(x.upper())


def arb_lower_point(x):
    """Return an Arb point at an already outward Arb lower endpoint."""
    return arb(x.lower())


def up_abs_arb(x):
    """Arb point not smaller than the absolute value enclosure of x."""
...
def arb_max_upper(values):
    """Fail-closed upper max: the sum of nonnegative uppers contains max."""
    vals = [arb(v) for v in values]
    if not vals:
        return arb(0)
    return arb_sum(vals)


def arb_definitely_less(x, y):
    try:
        return bool(x < y)
    except Exception:
        return False
...
def arb_nonnegative_radius(x):
    radius = up_abs_arb(x)
    if not arb_definitely_less(arb(-1), radius):
        raise RuntimeError("invalid Arb radius")
    return radius


def display_float_upper(x):
    """Binary64 display only; never feed back into enclosure arithmetic."""
    return hi(x)


def patch_quadratic_for_outward_arithmetic():
    """Replace v1.4 float radius helpers with Arb-only enclosure helpers."""
    Q = qengine.Quadratic

    def linear_radius_upper(self):
        return arb_sum(up_abs_arb(v) for v in self.a)

    def quadratic_radius_upper(self):
        return arb_sum(up_abs_arb(v) for row in self.q for v in row)

    def remainder_upper(self):
        return arb_upper_point(self.r)

    def deviation_upper(self):
        return self.linear_radius_upper() + self.quadratic_radius_upper() + self.remainder_upper()

    def enclosure(self):
        radius = arb_nonnegative_radius(self.deviation_upper())
        return self.c + base.acb(arb(0, radius), arb(0, radius))

    def inv(self):
        c_lo = arb_lower_point(abs(self.c))
        h = self.deviation_upper()
        if not arb_definitely_less(h, c_lo):
            raise ZeroDivisionError("quadratic inverse domain contains zero")
        l = self.linear_radius_upper()
        qrad = self.quadratic_radius_upper()
        r = self.remainder_upper()
        tail = qrad + r
        remainder = (
            r / (c_lo * c_lo)
        ...
        q = [[
            -self.q[i][j] / (self.c * self.c)
            + self.a[i] * self.a[j] / (self.c * self.c * self.c)
            for j in range(qengine.NV)
        ] for i in range(qengine.NV)]
        return Q(1 / self.c, [-v / (self.c * self.c) for v in self.a], q, arb_nonnegative_radius(remainder))
    ...
    def exp(self):
        ...
        tail = qrad + r
        ec = self.c.exp()
        ec_up = arb_upper_point(self.c.real.exp())
        remainder = ec_up * (r + (2 * l * tail + tail * tail) / 2 + h.exp() * h ** 3 / 6)
        q = [[ec * (self.q[i][j] + self.a[i] * self.a[j] / 2)
              for j in range(qengine.NV)] for i in range(qengine.NV)]
        return Q(ec, [ec * v for v in self.a], q, arb_nonnegative_radius(remainder))

    for func in (
        linear_radius_upper,
        quadratic_radius_upper,
        remainder_upper,
        deviation_upper,
        enclosure,
        inv,
        ...
        func._arithmetic = "arb_outward"  # audit marker for the active patch

    Q.linear_radius_upper = linear_radius_upper
    Q.quadratic_radius_upper = quadratic_radius_upper
    Q.remainder_upper = remainder_upper
    Q.deviation_upper = deviation_upper
    Q.enclosure = enclosure
    Q.inv = inv
    Q.exp = exp
    qengine.up_abs = up_abs_arb
    qengine.point_upper = arb_nonnegative_radius
    ENCLOSURE_RIGOUR_GUARDS.update({
...
def neumann_inverse(B, terms=14):
    ...
    for i in range(n):
        row = arb(0)
        for j in range(n):
            row += up_abs_arb(E[i, j])
        row_sums.append(row)
    q = arb_max_upper(row_sums)
    if not arb_definitely_less(q, arb(1)):
    ...
    for _ in range(1, terms + 1):
        power = power * E
        S = S + power
    tail = q ** (terms + 1) / (arb(1) - q)
    tail_ball = arb(0, arb_nonnegative_radius(tail))
    for i in range(n):
...
def setup_objects(workdir):
    ...
    patch_quadratic_for_outward_arithmetic()
...
def qneumann_inverse_near_identity(A, terms=3):
    ...
    for i in range(n):
        row = arb(0)
        for j in range(n):
            row += up_abs_arb(E[i][j].enclosure())
        row_sums.append(row)
    q = arb_max_upper(row_sums)
    if not arb_definitely_less(q, arb(1)):
    ...
    tail=q**(terms+1)/(arb(1)-q)
    tail_radius = arb_nonnegative_radius(tail)
    for i in range(n):
        for j in range(n): S[i][j].r += tail_radius
    return S,q,tail


neumann_inverse._norm_arithmetic = "arb_outward"
neumann_inverse._tail_arithmetic = "arb_outward"
qneumann_inverse_near_identity._norm_arithmetic = "arb_outward"
qneumann_inverse_near_identity._tail_arithmetic = "arb_outward"
...
def picard_step(model, old, xm, xr, h, beta, max_iter=18):
    ...
    for iteration in range(1, max_iter+1):
        ...
        if contained(pm, pr, bm, br):
            ...
            q_display = display_float_upper(arb_max_upper([q0, iq]))
            tail_display = display_float_upper(arb_max_upper([tail0, itail]))
            return ym, yr, {"iterations": iteration, "dL_upper": hi(dL),
                            "dV_upper": hi(dV), "regular": bool(ok and ok0),
                            "inverse_neumann_defect_upper": q_display,
                            "inverse_series_tail_upper": tail_display,
...
def main(argv=None):
    ...
    ap.add_argument("--report", default="/tmp/c4_e2b_b_eight_chart_flowpipe_v0_3_5_1.json")
