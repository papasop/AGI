#!/usr/bin/env python3
"""C4-E2b rigorous affine-correlation-preserving handoff v0.3.5.1.
...
TITLE = "C4-E2b AFFINE-CORRELATED HANDOFF SUBDIVISION RIGOROUS"
VERSION = "0.3.5.1"
...
CANDIDATE_NAME = "c4_e2b_local_recenter_affine_handoff_v0_3_3_1.json"
DIAGNOSTIC_NAME = "c4_e2b_handoff_controller_covariance_v0_3_4_2.json"
...
def interval_field(parent, model, parameter_chart, mid, rad, beta,
    ...
    q_display = parent.display_float_upper(q)
    tail_display = parent.display_float_upper(tail)
    rigour_guards = getattr(parent, "ENCLOSURE_RIGOUR_GUARDS", {})
    q_method = parent.qengine.Quadratic.deviation_upper
    neumann_method = parent.qneumann_inverse_near_identity
    rigour = {
        "no_float_in_quadratic_radius_path": (
            getattr(q_method, "_arithmetic", None) == "arb_outward"
            and rigour_guards.get("quadratic_radius_arithmetic") == "arb_outward"
        ),
        "no_float_in_neumann_norm_path": (
            getattr(neumann_method, "_norm_arithmetic", None) == "arb_outward"
            and rigour_guards.get("neumann_norm_arithmetic") == "arb_outward"
        ),
        "no_float_in_neumann_tail_path": (
            getattr(neumann_method, "_tail_arithmetic", None) == "arb_outward"
            and rigour_guards.get("neumann_tail_arithmetic") == "arb_outward"
        ),
        "q_definitely_below_one": bool(parent.arb_definitely_less(q, parent.arb(1))),
        "tail_is_arb_enclosure": isinstance(tail, parent.arb),
        "controller_inverse_enclosure_constructed": Ainv is not None,
    }
    finite = den_ok and parent.finite_ball(dL) and parent.finite_ball(dV)
    finite = finite and all(rigour.values()) and math.isfinite(q_display) and math.isfinite(tail_display)
    return dL, dV, bool(finite), q_display, tail_display, rigour
...
def leaf_record(parent, model, source, target, mid, rad, beta, depth,
    ...
    dL, dV, regular, q, tail, rigour = interval_field(
...
def main(argv=None):
    ...
    ap.add_argument("--report", default="/tmp/c4_e2b_affine_correlated_handoff_v0_3_5_1.json")
    ...
    protocol = {
        "version": VERSION,
        "repository_base_commit": "805ef2e62cdaec81f744f881d16806e74121b696",
    ...
    gates = {
        ...
        "all_leaf_enclosure_rigour_guards_pass": bool(
            all(all(r["enclosure_rigour"].values()) for r in accepted) and not blocked
        ),
    }
    ...
    report = {
        ...
        "enclosure_rigour": protocol["enclosure_rigour"],
        "required_next_step": (
