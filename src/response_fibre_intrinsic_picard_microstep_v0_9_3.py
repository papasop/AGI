#!/usr/bin/env python3
"""Arb intrinsic six-dimensional Picard microstep verifier v0.9.3.

This revision starts from the centered response increment introduced in
v0.9.2, certifies a complex parametric graph b=psi(a), encloses Dpsi, pulls the
Euclidean metric and L6 gradient back to the six tangent coordinates, and
closes a deliberately tiny Picard step by a Cauchy Lipschitz majorant.

The parameter residual is enclosed by the exact mean-value identity

    B(R(theta0 + T*a) - R(theta0))
      = integral_0^1 B J(theta0 + s*T*a) T*a ds,

so its constant term is exactly zero.  A validated ODE is claimed only when
the graph, metric, analytic square-root branch, Picard contraction,
self-mapping, response preservation and uniform descent gates all pass.
"""
...
TITLE = "RESPONSE-FIBRE ARB INTRINSIC SIX-DIMENSIONAL PICARD MICROSTEP"
VERSION = "0.9.3"
IMPLEMENTATION_REVISION = "complex-fibre-graph-pullback-cauchy-picard"
...
def injection_v093_picard() -> str:
    code = r'''
import os

v093_child = int(os.environ["V093_CHILD_INDEX"])
v093_outer_radius = ap(os.environ["V093_OUTER_RADIUS"])
v093_inner_radius = ap(os.environ["V093_INNER_RADIUS"])
v093_normal_radius = ap(os.environ["V093_NORMAL_RADIUS"])
v093_time_step = ap(os.environ["V093_TIME_STEP"])
v093_neumann_terms = int(os.environ["V093_NEUMANN_TERMS"])
v093_output = Path(os.environ["V093_OUTPUT"])
v093_output.mkdir(parents=True, exist_ok=True)


def v093_matvec(matrix, vector):
    return [
        sum((matrix[r][c] * vector[c] for c in range(len(vector))), acb(0))
        for r in range(len(matrix))
    ]
...
def v093_transpose(matrix):
    return [list(column) for column in zip(*matrix)]


def v093_inf_matrix(matrix):
    return max(
        (sum((upper_point(x) for x in row), arb(0)) for row in matrix),
        default=arb(0),
    )
...
def v093_complex_phase_box(base, tangent, normal, tangent_radius, normal_radius):
    phases = []
    for row in range(CONTROL_DIMENSION):
        radius = tangent_radius * sum(
            (upper_point(tangent[row][c])
        ...
        # A rectangular complex box is a formal superset of the required
        # complex polydisc.  This is conservative but dependency safe.
        phases.append(base[row] + acb(ball(0, radius), ball(0, radius)))
    return phases


def v093_midpoint_inverse(matrix, label):
    midpoint = np.asarray(
        [
            [midpoint_radius(matrix[r][c].real)[0]
    ...
    try:
        ...
        raise ArithmeticError(f"{label} midpoint inverse failed") from exc
    return [
        [acb(ap(float(inverse[r, c]))) for c in range(inverse.shape[1])]
        for r in range(inverse.shape[0])
    ]


def v093_defect(preconditioner, matrix):
    product = v093_matmul(preconditioner, matrix)
    return [
        [acb(int(r == c)) - product[r][c] for c in range(len(product[0]))]
        for r in range(len(product))
    ]


def v093_solve_matrix(matrix, preconditioner, right):
    """Neumann-enclose matrix^{-1} right column by column."""
    defect = v093_defect(preconditioner, matrix)
    q = v093_inf_matrix(defect)
    if not q < arb(1):
        raise ArithmeticError(
            f"Neumann matrix solve unresolved: defect={upper_float(q):.6e}"
        )
    base = v093_matmul(preconditioner, right)
    term = [list(row) for row in base]
    solution = [
        [acb(0) for _ in range(len(base[0]))] for _ in range(len(base))
    ]
    for _ in range(v093_neumann_terms):
        solution = [
            [solution[r][c] + term[r][c] for c in range(len(base[0]))]
            for r in range(len(base))
        ]
        term = v093_matmul(defect, term)
    tails = []
    factor = q ** v093_neumann_terms / (1 - q)
    for column in range(len(base[0])):
        base_upper = max(
            (upper_point(base[row][column]) for row in range(len(base))),
            default=arb(0),
        )
        tails.append(factor * base_upper)
    return [
        [add_disk(solution[r][c], tails[c]) for c in range(len(base[0]))]
        for r in range(len(base))
    ], q, max(tails, default=arb(0))


def v093_solve_vector(matrix, preconditioner, right):
    solved, q, tail = v093_solve_matrix(
        matrix, preconditioner, [[value] for value in right]
    )
    return [row[0] for row in solved], q, tail


def v093_run():
    tangent_dimension = CONTROL_DIMENSION - RESPONSE_DIMENSION
    if not 0 <= v093_child < CHILD_BOXES:
        raise ValueError("child index outside 0..15")
    if not (
        arb(0) < v093_inner_radius < v093_outer_radius
        and v093_normal_radius > arb(0)
        and v093_time_step > arb(0)
    ):
        raise ValueError(
            "require 0 < inner radius < outer radius and positive normal/time radii"
        )

    center = child_centers[v093_child]
    half_width = child_half_width
    center_record = audit_box(chart, center, half_width)
    if not center_record.get("stage_a_pass", False):
        raise ArithmeticError("frozen child Stage A failed")
    ...
    base_phases, _ = phase_and_derivative_at(chart, acb(ap(center)), True)
    frame = v093_frame(center)
    whitener = frame["whitener"]
    tangent = frame["tangent"]
    normal = frame["normal"]
    jacobian0 = frame["raw"]["jacobian"]
    gradient0 = frame["raw"]["gradient"]
    jw0 = v093_matmul(whitener, jacobian0)
    fb0 = v093_matmul(jw0, normal)
    fb_inverse = v093_midpoint_inverse(fb0, "normal derivative")

    tangent_domain = v093_complex_phase_box(
        base_phases, tangent, normal, v093_outer_radius, arb(0)
    )
    jacobian_tangent, _ = response_jacobian_and_gradient(tangent_domain, True)
    jw_tangent = v093_matmul(whitener, jacobian_tangent)
    fa_tangent = v093_matmul(jw_tangent, tangent)
    parameter_radii = [
        v093_outer_radius
        * sum((upper_point(fa_tangent[r][c]) for c in range(tangent_dimension)),
              arb(0))
        for r in range(RESPONSE_DIMENSION)
    ]
    parameter_residual = [add_disk(acb(0), radius) for radius in parameter_radii]

    # Start from the declared existence box, then repeatedly use the formal
    # Krawczyk image as a smaller root enclosure.  This is not heuristic
    # radius tuning: K(B) contains the unique root once K(B) is strictly
    # inside B.  Every refinement is re-audited on its own smaller box.
    current_normal_radius = v093_normal_radius
    graph_refinement_records = []
    graph_pass = False
    last_good_graph = None
    for refinement in range(6):
        full_domain = v093_complex_phase_box(
            base_phases, tangent, normal,
            v093_outer_radius, current_normal_radius
        )
        jacobian, gradient = response_jacobian_and_gradient(full_domain, True)
        jw = v093_matmul(whitener, jacobian)
        fa = v093_matmul(jw, tangent)
        fb = v093_matmul(jw, normal)
        fb_defect = v093_defect(fb_inverse, fb)
        fb_defect_upper = v093_inf_matrix(fb_defect)
        krawczyk_center = [
            -value for value in v093_matvec(fb_inverse, parameter_residual)
        ]
        krawczyk_radii = []
        for row in range(RESPONSE_DIMENSION):
            radius = upper_point(krawczyk_center[row])
            radius += current_normal_radius * sum(
                (upper_point(fb_defect[row][c])
                 for c in range(RESPONSE_DIMENSION)),
                arb(0),
            )
            krawczyk_radii.append(radius)
        image_radius = max(krawczyk_radii, default=arb(0))
        graph_utilization = image_radius / current_normal_radius
        rung_pass = bool(
            fb_defect_upper < ap(0.8) and graph_utilization < ap(0.95)
        )
        graph_refinement_records.append(
            {
                "iteration": refinement,
                "domain_radius": upper_float(current_normal_radius),
                "image_radius": upper_float(image_radius),
                "normal_derivative_defect_upper": upper_float(fb_defect_upper),
                "utilization": upper_float(graph_utilization),
                "pass": rung_pass,
            }
        )
        if not rung_pass:
            if last_good_graph is None:
                raise ArithmeticError(
                    "complex parametric graph unresolved: "
                    f"defect={upper_float(fb_defect_upper):.6e}, "
                    f"utilization={upper_float(graph_utilization):.6e}"
                )
            (
                current_normal_radius, full_domain, jacobian, gradient,
                jw, fa, fb, fb_defect, fb_defect_upper,
                krawczyk_radii, graph_utilization,
            ) = last_good_graph
            break
        if graph_utilization > ap("0.5") and last_good_graph is not None:
            # The preceding enclosure is already rigorous and has materially
            # better Krawczyk margin.  Do not trade robustness for a negligible
            # additional radius reduction.
            (
                current_normal_radius, full_domain, jacobian, gradient,
                jw, fa, fb, fb_defect, fb_defect_upper,
                krawczyk_radii, graph_utilization,
        ...
        # A five-percent outward margin avoids placing the next domain exactly
        # on the image bound.  Stop when another refinement is immaterial.
        candidate = image_radius * ap("1.05")
        if refinement == 5 or not candidate < current_normal_radius * ap("0.8"):
            break
        current_normal_radius = candidate

    minus_fa = [[-value for value in row] for row in fa]
    dpsi, dpsi_defect, dpsi_tail = v093_solve_matrix(
        fb, fb_inverse, minus_fa
    )
    graph_tangent = [
        [
            tangent[r][c]
            + sum((normal[r][k] * dpsi[k][c]
                   for k in range(RESPONSE_DIMENSION)), acb(0))
            for c in range(tangent_dimension)
        ]
        for r in range(CONTROL_DIMENSION)
    ]
    graph_tangent_t = v093_transpose(graph_tangent)
    metric = v093_matmul(graph_tangent_t, graph_tangent)
    metric_inverse = v093_midpoint_inverse(metric, "pullback metric")
    metric_defect = v093_defect(metric_inverse, metric)
    metric_defect_upper = v093_inf_matrix(metric_defect)
    if not metric_defect_upper < arb(1):
        raise ArithmeticError(
            "pullback metric inverse unresolved: "
            f"{upper_float(metric_defect_upper):.6e}"
        )

    pullback_gradient = v093_matvec(graph_tangent_t, gradient)
    metric_gradient, _, metric_solve_tail = v093_solve_vector(
        metric, metric_inverse, pullback_gradient
    )
    descent_square = sum(
        (pullback_gradient[i] * metric_gradient[i]
         for i in range(tangent_dimension)),
        acb(0),
    )
    if not descent_square.real > arb(0):
        raise ArithmeticError(
            "intrinsic projected-gradient square leaves the open right half-plane"
        )
    descent_norm = descent_square.sqrt()
    if descent_norm.contains(0):
        raise ArithmeticError("intrinsic projected-gradient norm contains zero")
    field = [-value / descent_norm for value in metric_gradient]
    field_sup = max((upper_point(value) for value in field), default=arb(0))
    descent_lower = arb(str(descent_square.real.lower())).sqrt()

    # Cauchy's estimate on the complex polydisc: for every component and
    # coordinate, |partial_j f_i| <= M/(R-r).  Thus the induced infinity-norm
    # Lipschitz bound is d*M/(R-r).
    cauchy_gap = v093_outer_radius - v093_inner_radius
    lipschitz = ap(tangent_dimension) * field_sup / cauchy_gap
    contraction = v093_time_step * lipschitz
    displacement = v093_time_step * field_sup
    self_mapping_utilization = displacement / v093_inner_radius

    gates = {
        "frozen_child_stage_a": True,
        "complex_parametric_fibre_graph": graph_pass,
        "implicit_graph_derivative_enclosed": bool(dpsi_defect < arb(1)),
        "pullback_metric_positive_definite": bool(metric_defect_upper < ap(0.8)),
        "analytic_normalization_branch": True,
        "intrinsic_projected_gradient_nonstationary": bool(
            descent_lower > ap(0.6)
        ),
        "cauchy_lipschitz_resolved": True,
        "picard_contraction": bool(contraction < ap(0.5)),
        "picard_self_mapping": bool(self_mapping_utilization < ap(0.95)),
        "exact_response_preservation": True,
        "uniform_strict_L6_descent": bool(descent_lower > ap(0.55)),
    }
    all_pass = all(gates.values())
    ...
    result = {
        "scientific_status": (
            ...
            if all_pass
            else "INTRINSIC_RESPONSE_FIBRE_ODE_MICROSTEP_INCONCLUSIVE"
        ),
        "all_gates_pass": bool(all_pass),
        "formal_interval_arithmetic": True,
        "arb_precision_bits": PRECISION_BITS,
        "validated_ODE_claimed": bool(all_pass),
        "ODE_existence_certified": bool(all_pass),
        "ODE_uniqueness_certified": bool(all_pass),
        "exact_response_preservation_certified": bool(all_pass),
        "uniform_L6_descent_certified_for_validated_solution": bool(all_pass),
        "global_flow_claimed": False,
        "child_index": v093_child,
        "tangent_dimension": tangent_dimension,
        "normal_dimension": RESPONSE_DIMENSION,
        "outer_complex_tangent_radius": float(v093_outer_radius),
        "inner_real_picard_radius": float(v093_inner_radius),
        "normal_graph_radius": float(v093_normal_radius),
        "refined_normal_graph_radius": upper_float(current_normal_radius),
        "selected_graph_refinement_iteration": next(
            (
                item["iteration"] for item in graph_refinement_records
                if abs(item["domain_radius"] - upper_float(current_normal_radius))
                <= max(1e-300, abs(item["domain_radius"]) * 1e-12)
            ),
            None,
        ),
        "graph_refinement_records": graph_refinement_records,
        "certified_time_step": float(v093_time_step),
        "midpoint_response_singular_values": [
            float(value) for value in frame["singular_values"]
        ],
        "complex_graph_normal_derivative_defect_upper": upper_float(
            fb_defect_upper
        ),
        "complex_graph_krawczyk_utilization": upper_float(graph_utilization),
        "centered_parameter_residual_upper": upper_float(
            max(parameter_radii, default=arb(0))
        ),
        "implicit_derivative_neumann_defect_upper": upper_float(dpsi_defect),
        "implicit_derivative_neumann_tail_upper": upper_float(dpsi_tail),
        "pullback_metric_neumann_defect_upper": upper_float(
            metric_defect_upper
        ),
        "pullback_metric_solve_tail_upper": upper_float(metric_solve_tail),
        "intrinsic_field_sup_norm_upper": upper_float(field_sup),
        "intrinsic_projected_gradient_norm_lower": upper_float(descent_lower),
        "cauchy_lipschitz_upper": upper_float(lipschitz),
        "picard_contraction_factor": upper_float(contraction),
        "picard_displacement_upper": upper_float(displacement),
        "picard_self_mapping_utilization": upper_float(
            self_mapping_utilization
        ),
        "uniform_dL6_dt_upper": -float(descent_lower),
        "gates": gates,
        "proof_identities": {
            "graph_equation": "B(R3(theta0+T*a+N*psi(a))-c)=0",
            "implicit_derivative": "Dpsi=-(B*DR3*N)^(-1)*(B*DR3*T)",
            "graph_tangent": "W=T+N*Dpsi",
            "pullback_metric": "H=W^T*W",
            "intrinsic_field": (
                "a_dot=-H^(-1)W^T grad(L6)/"
                "sqrt((W^T grad(L6))^T H^(-1)(W^T grad(L6)))"
            ),
            "response_preservation": "DR3*W=0",
            "lyapunov_identity": (
                "dL6/dt=-sqrt((W^T grad(L6))^T H^(-1)(W^T grad(L6)))"
            ),
        },
        "next_required_step": (
            "Chain adjacent certified microsteps with endpoint inclusion; "
            "do not call this a full-child or global flow theorem."
            if all_pass
            else "Refine only the first failed graph/metric/branch/Picard gate."
        ),
        "scope": (
            "one formally validated intrinsic six-dimensional ODE microstep "
            "inside child 15; not the whole child, ten-chart atlas or global fibre"
        ),
    }
    (v093_output / "intrinsic_picard_microstep_certificate.json").write_text(
        json.dumps(result, indent=2, sort_keys=True, allow_nan=False) + "\n",
        encoding="utf-8",
    )
    return result
...
banner("STAGE D - COMPLEX FIBRE GRAPH / PULLBACK / PICARD MICROSTEP")
try:
    v093_result = v093_run()
except Exception as exc:
    v093_result = {
        "scientific_status": "INTRINSIC_RESPONSE_FIBRE_ODE_MICROSTEP_INCONCLUSIVE",
        "all_gates_pass": False,
        "formal_interval_arithmetic": True,
        "validated_ODE_claimed": False,
        "error": f"{type(exc).__name__}: {exc}",
        "scope": "fail-closed intrinsic six-dimensional Picard microstep",
    }
    (v093_output / "intrinsic_picard_microstep_certificate.json").write_text(
        json.dumps(v093_result, indent=2, sort_keys=True, allow_nan=False) + "\n",
        encoding="utf-8",
    )

banner("FINAL RESULT v0.9.3")
print(json.dumps(v093_result, indent=2))
print("\nInterpretation")
if v093_result.get("all_gates_pass"):
    print("  PASS: one intrinsic six-dimensional response-fibre ODE microstep is validated.")
    print("  Exact response preservation and strict L6 descent hold on that solution.")
else:
    print("  INCONCLUSIVE: refine only the first reported graph/metric/branch/Picard gate.")
    print("  Do not claim a validated ODE.")
raise SystemExit(0)
'''
    return textwrap.indent(textwrap.dedent(code).strip("\n") + "\n", "    ")
...
'''
    ...
    return text.replace(needle, injection_v093_picard() + needle)
...
def main_v093() -> int:
    ...
    parser.add_argument("--v074-source")
    parser.add_argument("--child-index", type=int, default=15)
    parser.add_argument("--outer-radius", default="2e-11")
    parser.add_argument("--inner-radius", default="1e-11")
    parser.add_argument("--normal-radius", default="1e-10")
    parser.add_argument("--time-step", default="1e-14")
    parser.add_argument("--neumann-terms", type=int, default=20)
    parser.add_argument("--no-download", action="store_true")
    parser.add_argument(
        "--output",
        default="response_fibre_intrinsic_picard_microstep_v0_9_3_results",
    )
    args, ignored = parser.parse_known_args()
    if ignored:
        print(f"[notice] ignored notebook arguments: {ignored}")
    if not 0 <= args.child_index < 16:
        raise ValueError("--child-index must be between 0 and 15")
    try:
        outer_radius = Decimal(args.outer_radius)
        inner_radius = Decimal(args.inner_radius)
        normal_radius = Decimal(args.normal_radius)
        time_step = Decimal(args.time_step)
    except InvalidOperation as exc:
        raise ValueError("invalid decimal radius or time step") from exc
    if not (
        Decimal(0) < inner_radius < outer_radius
        and normal_radius > 0
        and time_step > 0
    ):
        raise ValueError(
            "require 0 < inner-radius < outer-radius and positive normal/time"
        )
    if args.neumann_terms < 1:
        raise ValueError("--neumann-terms must be positive")
    ...
    protocol = {
        ...
        "arb_precision_bits": 192,
        "purpose": (
            "certify a complex parametric response-fibre graph, its induced "
            "six-dimensional normalized descent field and one Picard microstep"
        ),
        "coordinate_model": "theta=theta0+T*a+N*psi(a)",
        "parameter_residual_identity": (
            "B(R(theta0+T*a)-R(theta0))="
            "integral_0^1 B*J(theta0+s*T*a)*T*a ds"
        ),
        "metric": "Euclidean metric pulled back by W=T+N*Dpsi",
        "picard_norm": "sup norm in six intrinsic coordinates",
        "cauchy_lipschitz": "L<=6*M/(outer_radius-inner_radius)",
        "child_index": args.child_index,
        "outer_complex_tangent_radius": str(outer_radius),
        "inner_real_picard_radius": str(inner_radius),
        "normal_graph_radius": str(normal_radius),
    ...
    started = time.time()
    with tempfile.TemporaryDirectory(prefix="response_fibre_v093_") as tmp:
        patched = Path(tmp) / "_v093_backend.py"
        patched.write_text(
            patch_frozen_source(source_path.read_bytes()), encoding="utf-8"
        )
        env = dict(os.environ)
        env.update(
            {
                "V093_CHILD_INDEX": str(args.child_index),
                "V093_OUTER_RADIUS": str(outer_radius),
                "V093_INNER_RADIUS": str(inner_radius),
                "V093_NORMAL_RADIUS": str(normal_radius),
                "V093_TIME_STEP": str(time_step),
                "V093_NEUMANN_TERMS": str(args.neumann_terms),
                "V093_OUTPUT": str(output),
            }
        )
        completed = subprocess.run(
            [
    ...
    certificate = output / "intrinsic_picard_microstep_certificate.json"
    if not certificate.is_file():
        raise RuntimeError(
            f"v0.9.3 backend exit={completed.returncode}; certificate missing"
        )
    result = json.loads(certificate.read_text(encoding="utf-8"))
    result.update(
        {
    ...
    write_json(output / "report.json", result)
    print("\n" + "=" * 120)
    print("FINAL WRAPPER RESULT v0.9.3")
    print("=" * 120)
    print(json.dumps(result, indent=2))
    return 0
